...
public class Thompson : MonoBehaviour
{
    ...
    float TiempoEntreRafagasInterno;
    public int cantidadDeDisparosPorRafagas=3;
    int disparosEnEstaRafaga;
    ...
    private void Start()
    {
        ...
        TiempoEntreRafagasInterno = tiempoEntreRafagas;
        disparosEnEstaRafaga = cantidadDeDisparosPorRafagas;
    }
    void Update()
    {
    ...
    void Disparar(RaycastHit2D[] Hit)
    {
        if (municion > 0)
        {
            ...
            {
                TiempoEntreRafagasInterno -= Time.deltaTime;
                SpriteTorreta.GetComponent<Animator>().SetBool("EnemigosEnRango", false);
                if (TiempoEntreRafagasInterno <= 0)
                {
                    TiempoEntreRafagasInterno = tiempoEntreRafagas;
                    disparosEnEstaRafaga = cantidadDeDisparosPorRafagas;
                }
